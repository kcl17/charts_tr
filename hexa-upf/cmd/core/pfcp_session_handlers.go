...
func HandlePfcpSessionEstablishmentRequest(conn *PfcpConnection, msg message.Message, addr string) (message.Message, error) {
	...
	logger.Pfcplog.Info("\n")
	logger.Pfcplog.Infof("%-47s", "------------------UE detected------------------")
	logger.Pfcplog.Info("\n")
...
func HandlePfcpSessionModificationRequest(conn *PfcpConnection, msg message.Message, addr string) (message.Message, error) {
	...
	logger.Pfcplog.Infof("Got Session Modification Request from: %s. \n", addr)
	...
	err := func() error {
		...
		for _, far := range req.UpdateFAR {
			...
			logger.Pfcplog.Infof("Updating FAR info: %d", farid)
			logger.Pfcplog.Infof("Action: %d", sFarInfo.FarInfo.Action)
			logger.Pfcplog.Infof("Remote IP: %d", sFarInfo.FarInfo.RemoteIP)
			logger.Pfcplog.Infof("Local IP: %d", sFarInfo.FarInfo.LocalIP)
...
func composeFarInfo(far *ie.IE, localIp net.IP, farInfo ebpf.FarInfo) (ebpf.FarInfo, error) {
	...
	if err == nil {
		...
		if outerHeaderCreationIndex == -1 {
			...
			logger.Pfcplog.Infof("No OuterHeaderCreation")
		} else {
